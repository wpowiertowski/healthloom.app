...
final class AppEnvironment {
    ...
    init(launchConfiguration: LaunchConfiguration = .current) {
        ...
        // Round-2 item 7: the catalogue stub (if any) rides in the
        // store so Settings' `.task` needs no branching.
        self.tipStore = TipStore(uiTestStub: launchConfiguration.tipsStub)
