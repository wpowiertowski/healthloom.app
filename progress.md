...
## WP-35 · Export & disconnect-and-wipe

GoogleHealthClient: `revokeRefreshToken()` (RFC 7009 POST to `oauth2.googleapis.com/revoke`, new `revocationEndpoint` config default; clears actor cache + token store in ALL outcomes via defer, `.nothingStored` short-circuits network, log-safe `.revocationFailed(status:)`). App `DataManagement/`: versioned `ExportBuilder` (LocalSample + profile + chat DTOs, sorted-keys/pretty/ISO-8601 golden snapshot), `HealthKitSourceDeleter` (bundle-ID equality filter, per-type outcomes, auth-gated live path after finding unauthorized queries hang unbounded on sim), `StoreDeleter` (main+wal+shm via CoreModel's single-sourced URL + tmp export sweep), `WipeCoordinator` (revoke → keys → HK → store → defaults, continue-all ledger, same-day persist dedupe). Wipe set derived from the authorized set (P0-mapped + writer workout-attachment incl. ALL distance buckets — hand-listing both over- and under-covered), never hand-listed. Settings UI: export row + ShareLink, nav-row wipe flow (confirm, HK opt-out, persistent ledger, contained done phase with no Back/Close until relaunch), humanized per-type retry guidance.
...
**Performance:** first-token prewarm pinned (onAppear warms exactly once); backfill memory audited structurally (per-chunk ModelContexts, one-chunk-per-type fairness, bounded result dicts — no accumulation shape; Instruments pre-release per test-plan §10).
